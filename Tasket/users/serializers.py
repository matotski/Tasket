...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_active_projects(obj):
        return [
            project.title
            for project in Project.objects.filter(users=obj, status="active")
        ]
    ...
    def get_all_projects(obj):
        return [
            project.title
            for project in Project.objects.filter(users=obj, status="archived")
        ]
...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for user_data in users_data:
            ...
            UserProjectRole.objects.create(
                project=project, user_id=user_id, role_id=role_id
            )
        return project
    ...
    def update(self, instance, validated_data):
        ...
        for user_data in users_data:
            user_id = user_data["user"]
            role_id = user_data.get("role")
            UserProjectRole.objects.update_or_create(
                project=instance, user_id=user_id, defaults={"role_id": role_id}
            )
        return instance
        ...
        return [
            {
                ...
                "role": RoleSerializer(user_role.role).data["id"]
                if user_role.role
                else None,
            }
...
class TaskSerializer(serializers.ModelSerializer):
    tester = serializers.PrimaryKeyRelatedField(
        queryset=UserProjectRole.objects.filter(
            Q(role__name__icontains="тестировщик")
            | Q(role__name__icontains="Тестировщик")
        ),
        allow_null=True,
    )
    ...
    def get_comment(obj):
        return [
            {"created_at": comment.created_at, "text": comment.text}
